...
export class GCPLog {
  ...
  now: number;
  elapsedHrTime: [number, number] | undefined;
  elapsedTimeInMs: number | undefined;
  requestId: string | undefined;
  containerName: string;
  jobId: string;
  constructor(containerName: string, jobId: string) {
  ...
  async endTimeAndLog(requestId: string): Promise<void> {
    this.endTime();
    ...
    const entry = gcpLogging.entry(
      {
        ...
        severity: 'INFO',
        labels: {
          latency: this.elapsedTimeInMs?.toString() as string,
        },
      },
      {
